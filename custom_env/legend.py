...
class Legend:
    ...
    def draw(
        ...
        for item in items:
            if item == "zone_count":
                ...
                self._draw_custom_zone_count(
                    self.frame, y_pos, inputs[item],
                    inputs["bbox_labels"],
                    names
                ) #
            else:
    ...
    # CUSTOM START
    def _draw_custom_zone_count(
        self, frame: np.ndarray, y_pos: int, counts: List[int],
        bbox_labels: List[str], names: List[str] #
    ) -> None:
        """Draw zone counts by class ids of all zones onto frame image
        ...
        # slice bbox_labels into zones #
        iterable = iter(bbox_labels) #
        labels_by_zone = [list(islice(iterable, sl)) for sl in np.array(counts)] #
        ...
        # count_arr = [[0] * len(ids)] * len(counts)
        for i, count in enumerate(counts):
            y_pos += 20
            cv2.rectangle(
                frame,
            ...
            count_dict = {n:labels_by_zone[i].count(n) for n in names} #
            
            text = f" ZONE-{i+1}: {count_dict}" # amended
            cv2.putText(
                frame,
                text,
                (40, y_pos),
    ...
    # CUSTOM START
    # def _draw_zone_count(
        # self, frame: np.ndarray, y_pos: int, counts: np.array
    # ) -> None:
        # """Draw zone counts of all ids in all zones onto frame image
        # Args:
            # frame (np.array): image of current frame
            # y_pos (int): y position to draw the count info text
            # counts (list): list of 2D array of ids by zone counts,
                            # eg. 4 ids by 2 zones
                            # [[z1_id1, z1_id2, z1_id3, z1_id4],
                            #  [z2_id1, z2_id2, z2_id3, z2_id4]]
        # """
        # text = "-ZONE COUNTS-"
        # cv2.putText(
            # frame,
        ...
        # for i, count in enumerate(counts):
            ...
            # )
# 
            # draw various ids' counts in each zone
            # id_x = 0
            # for i, id in enumerate(count):
                # text = f" ID: {id}"
                # cv2.putText(
                    # frame,
                    # text,
